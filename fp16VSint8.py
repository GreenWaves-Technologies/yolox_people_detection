...
from utils import build_graph, make_parser, onnx_layer_output, clip_stats
...
if __name__ == "__main__":
    ...
    with open(f"./stats_val.pickle", "rb") as f:
        # stats = pickle.load(f)
    ...
    flaot_names = OrderedDict()
    for i, output in enumerate(outputs_nntool_float):
        flaot_names[graph_fp16[i].name] = output[0]
    ...
    names = flaot_names.keys() & int8_names.keys()

    # calucate qsnr for each layer
    for name in flaot_names:
        if name in int8_names :
            if "constant" in name or "weights" in name:
                continue
            if flaot_names[name].shape == int8_names[name].shape:
                qsnr_ = qsnr(flaot_names[name], int8_names[name])
